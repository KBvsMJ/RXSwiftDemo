    ...
    override func viewDidLoad() {
       ...
       _ = btnBinds.rx.tap.subscribe(onNext:{ [unowned self] in
        ...
        _ = btnTableView.rx.tap.subscribe(onNext:{ [unowned self] in
            let sb:UIStoryboard! = UIStoryboard.init(name: "Main", bundle: nil)
            let tbCtl = sb.instantiateViewController(withIdentifier: "TableViewController")
            self.navigationController?.pushViewController(tbCtl, animated: true)
        })
        _ = btnContentOffset.rx.tap.subscribe(onNext:{
            [unowned self] in
            let sb:UIStoryboard! = UIStoryboard.init(name: "Main", bundle: nil)
            let sectionTbCtl = sb.instantiateViewController(withIdentifier: "SectionTableViewController")
            self.navigationController?.pushViewController(sectionTbCtl, animated: true)
